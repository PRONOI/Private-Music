...
async def thumb_init(videoid):
    try:
        if os.path.isfile(f"FallenMusic/Cache/{videoid}.jpg"):
            return f"FallenMusic/Cache/{videoid}.jpg"
        ...
        if 1==1:
                ...
                async with session.get(f"http://img.youtube.com/vi/{videoid}/maxresdefault.jpg") as resp:
                    if resp.status == 200:
                        f = await aiofiles.open(
                            f"FallenMusic/Cache/thumb{videoid}.jpg", mode="wb"
                        )
                        await f.write(await resp.read())
                        await f.close()

            youtube = Image.open(f"FallenMusic/Cache/thumb{videoid}.jpg")
            ...
            # fonts
            font1 = ImageFont.truetype('FallenMusic/Utilities/font.ttf', 30)
            font2 = ImageFont.truetype('FallenMusic/Utilities/font.ttf', 70)
            font3 = ImageFont.truetype('FallenMusic/Utilities/font.ttf', 40)
            font4 = ImageFont.truetype('FallenMusic/Utilities/font.ttf', 35)

            image4 = ImageDraw.Draw(image2)
            image4.text((10, 10), "ALONE VIBE", fill="white", font = font1, align ="left") 
            ...
            image2.save(f"FallenMusic/Cache/{videoid}.jpg")
            file = f"FallenMusic/Cache/{videoid}.jpg"

            return file
    except Exception as e:
        return "FallenMusic/Utilities/Audio.jpeg"
